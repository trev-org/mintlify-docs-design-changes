...
* **BWOIExample:** Demonstrates how to build an MSBuild project without installing the Microsoft Game Development Kit (GDK). Also includes a Dockerfile, demonstrating how to build in a [Windows container](/virtualization/windowscontainers/) using [Docker](/virtualization/windowscontainers/manage-docker/configure-docker-daemon).
