...
- Xbox services, Configuring development network access
- Xbox services, Configuring your development network access for developing Microsoft Game Development Kit titles
- Xbox services, required domains
- Xbox services, networks
- Xbox services, Wake on LAN
- Xbox services, Submission Validator tool
ms.topic: article
edited: 07/27/2020
ms.date: 10/11/2022
...
## Required server-to-server Xbox network configuration  
   
If your title makes server-to-server Xbox network (also known as Xbox Live) calls, update the routing configurations and/or access control lists (ACLs). This ensures that you can connect to the Xbox network datacenter from your services. 
...
Failure to account for the new CIDR blocks could prevent your services from interacting with Xbox services .
...
### Required configuration for using the Submission Validator tool

The required port for using the Submission Validator tool is 443.  

The required domain for using the Submission Validator tool is https://*.xboxlive.com.  

<a id="wol_support"></a>  

### Required ports for Wake on LAN support for Xbox dev kits  

Some functionality requires the ability to send *Wake on LAN* magic packets to your dev kit. This includes the [Reboot (xbreboot.exe) (NDA topic)](../../../../tools/tools-console/commandlinetools/xbreboot.md) command-line tool and some functionality of [Xbox Manager (NDA topic)](../../../../tools/tools-console/xbom/xbom.md). Wake on LAN is sent on the following UDP ports.
...
Most Xbox services (including sign-in and authentication) currently only provide IPv4 endpoints. As a result development and testing should only be performed on networks with IPv4 (and optionally IPv6) connectivity. Pure IPv6 networks are currently not supported. Development tools and console socket APIs support IPv6 connectivity. 


## Next steps

After configuring the network access for you development PC you should create a new project to verify your GDK installation by proceeding to [Developing a new title with the GDK](../../../get-started/overviews/developing-new-titles-on-gamecore.md).  
