...
- Running PC samples
- Samples, Running PC
- Xbox services, Samples
- Xbox services, PC samples
- Xbox services, Running PC samples
ms.topic: article
edited: 03/24/2020
ms.date: '08/01/2022'
...
You can download samples for the Microsoft Game Development Kit (GDK) from [GitHub](https://github.com/microsoft/Xbox-GDK-Samples). They aren't included in the Microsoft Game Development Kit (GDK) installation.
...
1.  Install the required development tools: <br>Visual Studio 2019 or Visual Studio 2022, Windows 10 SDK, and the Microsoft Game Development Kit (GDK).
...
![A simple triangle](../../../../media/public-images/simpletriangle.png)

Because the samples use DirectX 12, they require Windows 10 or Windows 11, a video card, and a video driver capable of DirectX 12.
...
By default, the samples are set to use the Visual Studio 2019 Platform Toolset `v142`. When using Visual Studio 2022, you can either build the samples by using the `v142` setting or retarget the samples to `v143`.

We recommend that when you install Visual Studio 2022, you include the MSVC v142 - VS 2019 C++ x64/x86 build tools optional component (`Microsoft.VisualStudio.ComponentGroup.VC.Tools.142.x86.x64`).
...
* The *Assets* folder contains local media, which includes the startup screen and logos used for packaging. Samples also include a *MicrosoftGameConfig.mgc* file.
...
The primary utility code that's used by the samples is the DirectX Tool Kit for DX12, which is available on [GitHub](https://go.microsoft.com/fwlink/?LinkID=615561). A few samples use DirectXTex, which is also available on [GitHub](https://go.microsoft.com/fwlink/?LinkId=248926). For documentation, see the appropriate project wiki. Microsoft Game Development Kit (GDK) support in DirectX Tool Kit and DirectXTex are subject to the MIT license and are part of the core project.

Samples also use source code in the ATG Tool Kit, in the *ATGTK* folder. This source code is MIT-licensed code (as noted in the individual headers) and is intended for use only in the samples.
