---
author: M-Stahl
title: "Setting up a remote test PC"
description: "Describes how to set up a PC for use with remote testing."
kindex:
- Setting up a remote test PC
- remote test PC set up
- PC, remote test set up
ms.author: douglau
ms.topic: article
edited: 10/21/2019
ms.date: '08/01/2022'
...
If your title is using MicrosoftGame.config to install framework dependencies, these need to be sideloaded onto machines configured to use the developer sandbox.  For more information, see [Framework package dependencies](../../../features/common/packaging/packaging-framework-packages.md).
...
1. Go to the **Select the features you want to install** page as shown in the following figure.  
    ...
    ![Select the features you want to install page](../../../../media/public-images/gaming-runtime-dependencies.png)  
...
1. Go to the GamingServices.appxbundle package, which is located by default at the following path:  
    ...
    > [!NOTE] 
    > In the previous example path, *build_number* represents the build that's installed on your system (for example, 190700).  
...
[Remote Debugging a C++ project in Visual Studio](/visualstudio/debugger/remote-debugging-cpp?view=vs-2019)  