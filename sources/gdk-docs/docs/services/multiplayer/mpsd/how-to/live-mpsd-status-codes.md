...
description: "Provides the status codes that are returned from Xbox services when a multiplayer session is requested."
kindex:
- Multiplayer session status codes
- multiplayer 2015
- status codes, session
- Xbox services, Status codes
- Xbox services, Status codes session
- Xbox services, Multiplayer session status codes
- Xbox services, Multiplayer session status
- Xbox services, Multiplayer 2015
author: joannaleecy
ms.topic: article
...
This topic provides the status codes that are returned from Xbox services when a multiplayer session is requested.
...
| 429 | Too many requests | application/json | Message | The service call was throttled because of exceeding the Fine-Grained Rate Limiting (FGRL) restrictions. For more information, see [Fine-Grained Rate Limiting](../../../develop/best-practices/live-fine-grained-rate-limiting.md). |
