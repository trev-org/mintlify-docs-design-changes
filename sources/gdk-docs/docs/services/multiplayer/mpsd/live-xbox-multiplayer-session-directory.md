...
description: "Creating multiplayer sessions by using the Xbox services Multiplayer Session Directory (MPSD) service."
kindex:
- Multiplayer Session Directory
- Xbox services, Multiplayer Session Directory
- Xbox services, MPSD
- Xbox services, session template
- Xbox services, contract schema
- Xbox services, member initialization
- Xbox services, match ticket
- Xbox services, Quality of Service (QoS)
- Xbox services, Xbox Cloud Compute
author: joannaleecy
ms.topic: article
...
This article provides an overview of multiplayer session creation using the Xbox services Multiplayer Session Directory (MPSD) service, on platforms later than Xbox 360.

This article is directed primarily toward Xbox title developers who submit their session templates directly to Partner Center.
...
The client-side Xbox Services API (XSAPI) libraries currently use contract version 104 when communicating to the MPSD services.
...
The previous revision of this document updated information regarding server sessions and added a new section about XSAPIs and RESTful service calls.
...
-   Server sessions
    ...
    -   Game sessions created and used through Xbox Compute processing.
...
### XSAPIs and RESTful service calls

There are two ways to interface with the Xbox services Sessions and Matchmaking services.
...
For Xbox One and later, the first way to interface with the Xbox services Sessions and Matchmaking services is to use standard HTTPS calls to the RESTful Xbox services URIs.
...
For a list of these URIs, see [Xbox services RESTful Reference](../../../reference/live/rest/atoc-xboxlivews-reference.md).

#### Using the XSAPI wrappers for RESTful services

The second way to interface with the Sessions and Matchmaking services is to use XSAPIs, which act as wrappers for the RESTful service URIs.
These allow for a more traditional approach to using APIs in code without having to handle HTTPS traffic for each call.

More information about the XSAPIs, see the [xbox-live-api GitHub page](https://github.com/microsoft/xbox-live-api).
...
* Xbox Compute SDK Reference - APIs for creating game server instances for Xbox Compute.
...
![Hopper edit dialog box](../../../../media/public-images/live/whitepapers/mpsd_image1.png)
...
![Member initialization flowchart](../../../../media/public-images/live/whitepapers/mpsd_image2.png)
