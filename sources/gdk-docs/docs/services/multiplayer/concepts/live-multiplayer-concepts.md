...
- Xbox services, Multiplayer concepts overview
- Xbox services, Multiplayer system
- Xbox services, Multiplayer session
- Xbox services, MPSD
- Xbox services, Multiplayer Session Directory
- Xbox services, SmartMatch
- Xbox services, SmartMatch matchmaking
- Xbox services, matchmaking
- Xbox services, Quality of Service
- Xbox services, QoS
author: joannaleecy
ms.topic: article
...
This topic describes a number of important multiplayer terms and concepts that are used frequently in the Xbox services documentation.
Having a good grasp of them helps you understand how Xbox services multiplayer works.


## Multiplayer session

A *multiplayer session* represents a group of Xbox services players and the properties that are associated with them.
The session is created and maintained by titles. It's represented as a secure JSON document that resides in the Xbox services cloud.
...
Each session is based on a *session template*. It's defined by the game developer and is configured in the Xbox services configuration for a title instance.

A title can create and update a session, but it can't directly delete a session.
Instead, after all players are removed from a session, the Xbox services Multiplayer service automatically deletes the session after a specified time-out.
...
A *session member* is an Xbox services player that's part of a session.
...
The Xbox services Multiplayer service operates in the Xbox services cloud. The service centralizes a game's multiplayer system metadata across multiple clients.
...
Session handles allow Xbox services members to join existing sessions.
...
Xbox services provides an invite system that's supported by the Multiplayer service.
...
In addition to invites, Xbox services also provides a shell option for players to join an active gameplay session of friends or other known players.
...
*SmartMatch* is the name of the Xbox services service for anonymous matchmaking.
