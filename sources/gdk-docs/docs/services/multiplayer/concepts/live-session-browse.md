...
- session browse
- Xbox services, Multiplayer session browse
- Xbox services, multiplayer
- Xbox services, session browse
- Xbox services, search
author: joannaleecy
ms.topic: article
...
The search handle is stored in the [Multiplayer Service Directory (MPSD)](../mpsd/live-mpsd-overview.md), which maintains the search handles for the title. 
...
The `userAuthorizationStyle` capability is only required for UWP games, but we recommend implementing them for all Xbox games, including Microsoft Game Development Kit (GDK) games. This ensures future portability for your title.
...
You can set these capabilities in the session template when you configure your Xbox services.
...
> [!NOTE]
> If an owner of a session has an Xbox member blocked, that member can't join the session.
...
To create a search handle for a session by using the Xbox Services API (XSAPI), call the [XblMultiplayerCreateSearchHandleAsync](../../../reference/live/xsapi-c/multiplayer_c/functions/xblmultiplayercreatesearchhandleasync.md) method.
 ...
 * The `numbers` are numeric variables. The XSAPIs retrieve number values as type float. 
...
In the XSAPIs, you can set the search attributes, such as tags and metadata, by using the [XblMultiplayerCreateSearchHandleAsync](../../../reference/live/xsapi-c/multiplayer_c/functions/xblmultiplayercreatesearchhandleasync.md) method.  
...
In the XSAPIs, these details, along with the search attributes, are included in the `XblMultiplayerSearchHandle` results that return after a search query.
...
In the XSAPIs, remove a search handle by using the [XblMultiplayerDeleteSearchHandleAsync](../../../reference/live/xsapi-c/multiplayer_c/functions/xblmultiplayerdeletesearchhandleasync.md) method.  

### Create a search handle with metadata

The following code shows how to create a search handle for a session by using the Xbox multiplayer APIs.
...
The Xbox service only returns the first 100 results that match the search query.
