...
- matchmaking
- SmartMatch
- Xbox services, Using SmartMatch matchmaking
- Xbox services, Matchmaking
- Xbox services, SmartMatch
- Xbox services, Multiplayer
- Xbox services, Multiplayer matchmaking
author: joannaleecy
ms.topic: article
...
[Performing matchmaking for games by using Xbox Compute](#performing-matchmaking-for-games-using-xbox-live-compute)  
[See also](#see-also)  
...
![Image of a matchmaking flow chart that illustrates the SmartMatch matchmaking process](../../../../../media/public-images/live/multiplayer/Multiplayer_2015_SmartMatch_Matchmaking.png)
...
The matchmaking process "flattens" per-member, each into a single ticket-level attribute, based on the `flatten` method that was specified for that attribute in the Xbox services configuration for the hopper.
...
1. All players perform initialization on the target session as previously mentioned. However, because the players will be connecting to Xbox Compute, they don't perform QoS with one another to confirm connectivity.  
