...
`connectionString` | A title-defined string that's opaque to MPA. Contains information that a title can use to connect to another player. | Must be less than 512 characters.
...
`groupId` | A title-defined ID. Players with the same `groupId` are recognized in MPA to be playing together. | Must be less than 128 characters and only be used for group identity.
