...
- sign-in
- Xbox services, Troubleshooting Xbox services sign-in
- Xbox services, sign-in
- Xbox services, sandbox
- Xbox services, service configuration
ms.topic: article
...
You should familiarize yourself with the concept of sandboxes and how they pertain to Xbox services.
...
For Windows 10 Desktop, use the `SwitchSandbox.cmd` script that's located in the `Tools` directory of the Xbox SDK installation.
...
You cannot sign-in to Xbox network (also known as Xbox Live) in a given development sandbox for a title, unless that title is published to the same sandbox.

To publish the title to the same sandbox, see the section [Sandbox IDs](../../sandboxes/live-setting-up-sandboxes.md#sandbox-ids) in the article "Setting up sandboxes".
...
![0x8015DC12 Sign-In Error Screenshot](../../../../../media/public-images/live/identity/sign_in_error.png)

### 0x80860003 The application is either disabled or incorrectly configured

1. Try deleting your PFX file.

   ![pfx file in Solution Explorer](../../../../../media/public-images/live/identity/pfx_file.png)
...
2. Double-check that your `package.appxmanifest` is set to the same application identity as your title in Partner Center.
   ...
   ![Associate with store](../../../../../media/public-images/live/identity/appxmanifest_binding.png)
...
1. This could mean you're not using a test account to attempt sign-in, or your test account doesn't have access to the same sandbox you're signed in as.
   Double-check the instructions on creating test accounts in [Xbox services test accounts](../../../develop/test-accounts/live-test-accounts.md).
...
2. Double-check that your title is published to the sandbox that you are trying to use.
   See the section [Sandbox IDs](../../sandboxes/live-setting-up-sandboxes.md#sandbox-ids) in Setting up sandboxes.
