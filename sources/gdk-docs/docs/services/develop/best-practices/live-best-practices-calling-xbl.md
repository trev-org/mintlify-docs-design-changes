---
title: "Best practices for calling Xbox services"
description: "Best practices for calling Xbox Services APIs, such as proper calling patterns and retry logic."
kindex:
- Calling Xbox Live
- Best practices, Calling Xbox Live
- Xbox services, Best practices for calling Xbox services
- Xbox services, best practices
- Xbox services, calling Xbox services
- Xbox services, XSAPI
- Xbox services, REST endpoints
ms.topic: article
...
Regardless of how your code calls Xbox services, it is important to have proper calling patterns and retry logic.
...
This value indicates the Xbox services authentication token expired, so the Xbox Services API calls into the OS to refresh the token and then performs as a single retry.
...
This means by default each retry does an exponential back-off of 2, 4, or 8 or more seconds. It jitters the delay between the current and next back-off value based on the response time to further spread out load across the set of devices attempting the retry.
...
There are many real-world conditions that can result in a request to Xbox services to return failure codes, such as:
...
For details on best practices of error handling, see [Error handling](../../../archive/services-archive/live-error-handling-nav.md).
...
For example, with the Xbox service's profile service you can ask for a single user's profile or a set of users profiles.
...
The Real-Time Activity service exposes a web socket that sends a notification to clients when target resources change on the service.
...
To know what the information client is interested in, the client must first subscribe to the item over the web socket.
...
See [Social Manager](../../community/social-manager/live-social-manager-nav.md).
...
See [Multiplayer Manager](../../multiplayer/mpm/live-multiplayer-manager-nav.md).


## Throttling (fine grained rate limiting)

Xbox services have throttling in place to prevent any single device from putting extreme load on the service.
...
Another option for determining whether your title was throttled is to record a trace of the Xbox service calls and then analyze that trace using the [Xbox services Trace Analyzer tool](../../../tools/tools-services/live-trace-analyzer.md).

To record a trace, you can either use Fiddler to record a .SAZ file, or by using the built-in trace logging of XSAPI.
For more information about turning on traces in XSAPI, see the Xbox article [Xbox services Trace Analyzer (XblTraceAnalyzer.exe)](../../../tools/tools-services/live-trace-analyzer.md).
Once you have a trace, the Xbox services Trace Analyzer tool will warn you when it detects throttled calls.


## Is Xbox services up?

Xbox service is a collection of microservices that expose Xbox features such as profile, friends and presence, stats, leaderboards, achievements, multiplayer, and matchmaking.
There isn't a single server or endpoint that defines whether Xbox services is up.
If a single server goes down, the rest of the Xbox service's microservices are largely independent and should be operational.
...
So simply report to the user the last known presence, instead of reporting that Xbox network (also known as Xbox Live) is down.

Xbox services follows the consistency model of "eventual consistency".
