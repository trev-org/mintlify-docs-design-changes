...
description: "Describes how to troubleshoot Xbox services sign-in errors."
kindex:
- troubleshooting sandbox
- troubleshooting sign-in
- Xbox services, Troubleshooting Sign-in and Sandbox Errors
- Xbox services, troubleshooting sign-in
- Xbox services, troubleshooting sandbox
ms.author: gershp, ricardopezo
ms.topic: article
edited: 3/30/2026
...
- [Account and sandbox errors](#account-and-sandbox-errors)
- [Configuration errors](#configuration-errors)
- [Game-specific troubleshooting](#game-specific-troubleshooting)
- [Additional troubleshooting](#additional-troubleshooting)
- [Error code quick reference](#error-code-quick-reference)
...
> [!IMPORTANT]
> It is critical to have the XblPCSandbox.exe tool already installed.
> For more detail on how to obtain it, refer to [Sandboxes overview](../../fundamentals/sandboxes/live-setup-sandbox.md)
...
Compare the output with the sandbox shown on the **Gameplay settings** page in Partner Center.
You can do so by using https://partner.microsoft.com/en-us/xboxconfig?appid=[productId], replacing the [productId] parameter with your respective *Product Id*.
...
See [Sandboxes overview](../../fundamentals/sandboxes/live-setup-sandbox.md) for detailed instructions on switching sandboxes.
...
Additionally, check the **SCID** (Service Configuration ID). The SCID isn't in your `MicrosoftGame.config`—it's passed as a parameter to the [XblInitialize](../../../reference/live/xsapi-c/xbox_live_global_c/functions/xblinitialize.md) method. Find it under **Xbox services** > **Xbox Settings** in Partner Center.

> [!TIP]
> Use the **Store Association Wizard** in the [MicrosoftGame.config Editor](../../../features/common/game-config/microsoftgameconfig-editor.md) to link your game automatically and reduce the chance of typos.
...
If you could **not** sign in to the Xbox App (Step 1 of the [diagnostic flowchart](#diagnostic-flowchart)), the issue is with your sandbox switch, test account, or Xbox App itself.
...
| Cause | Fix |
|-------|-----|
| IDs in `MicrosoftGame.config` don't match Partner Center | See [checklist item 1](#-1-review_the_sandbox_Id_does_not_have_any_typos) and [checklist item 2](#-2-your-sandbox-is-correct)|
...
| Using an isolated sandbox instead of shared | Check sandbox type in Partner Center; switch to a shared sandbox unless you have a specific reason to use isolated. See [Isolated vs. shared sandboxes](#isolated-vs-shared-sandboxes). |
...
If you **can** sign in to the Xbox App but **cannot** sign in within your game or a sample project (Step 2 of the [diagnostic flowchart](#diagnostic-flowchart)), the issue is with your game's Xbox services configuration.

### PC Platform not enabled (0x87dd0005, AM_E_XAST_UNEXPECTED)
...
Use the **Store Association Wizard** in the [MicrosoftGame.config Editor](../../../features/common/game-config/microsoftgameconfig-editor.md) to link your game and autopopulate the correct values. Or manually verify each value against Partner Center using [checklist item 1](#-1-your-ids-match-partner-center).

### Incorrect SCID in XblInitialize

The SCID is **not** in `MicrosoftGame.config`—it's passed as a parameter to [XblInitialize](../../../reference/live/xsapi-c/xbox_live_global_c/functions/xblinitialize.md). Make sure you're passing the correct SCID from Partner Center (**Xbox services** > **Xbox Settings**).
...
If the Xbox App sign-in works **and** a sample project works with your game's IDs, but your game still can't sign in (Step 3 of the [diagnostic flowchart](#diagnostic-flowchart)), the issue is specific to your game's code or configuration.
...
- [Xbox Status page](https://support.xbox.com/xbox-live-status)
 ...
 > [!IMPORTANT]
 > **Known limitation (GDK 26.04):** `XblPCSandbox` fails to switch
 > sandboxes when run over an SSH session. Run `XblPCSandbox` from a
...
|------------|------|-------------|-----|
| `0x8015DC12` | User not in sandbox | Mismatched IDs, no sandbox access, expired access for the test account to the sandbox, or isolated sandbox | See [checklist](#preflight-checklist); verify IDs, account access, and sandbox type |
...
| `0x80004005` | Generic sign-in failure | Sandbox or account misconfiguration; can also indicate a service-side issue | Work through the [Preflight checklist](#preflight-checklist); verify sandbox ID, test account access, and account expiration. If the issue persists, try restarting your PC and re-switching sandboxes. |
...
- [Sandboxes overview](../../fundamentals/sandboxes/live-setup-sandbox.md)
- [Advanced sandboxes conceptual overview](../../fundamentals/sandboxes/live-advanced-sandboxes.md)
- [Test accounts](../test-accounts/live-test-accounts.md)
