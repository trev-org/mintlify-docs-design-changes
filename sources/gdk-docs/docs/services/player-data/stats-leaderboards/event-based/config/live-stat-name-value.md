...
- Xbox services, Adding a field value to a state rule
...
Suppose your game sends a `GameEnd` event to Xbox services. The event's `Map` field value is 1. The `Gold` field value is 100.
...
| Item | Type | Notes |
| --- | --- | --- |
| `GameEnd` | An event | Sent by your game to Xbox services |
