...
- Xbox services, Portal configuration of event-based Leaderboards
- Xbox services, Configure event-based Leaderboards at Partner Center
author: joannaleecy
ms.topic: article
...
The Xbox services Leaderboard service provides the capability for storing and retrieving global leaderboards for Xbox services titles. Each leaderboard score is automatically updated by the Xbox data platform to reflect the appropriate values of the score's underlying stat instances. You can maintain multiple leaderboards for each title.
...
> [!IMPORTANT] 
> There's no support for changing or deleting a leaderboard that's been published. This means that you must create a new leaderboard with a different leaderboard ID. Changing the sort order or the underlying instance of a leaderboard's stat might cause data loss. Avoid reusing leaderboard IDs that have already been configured and published.
