...
- Xbox services, Title-managed achievements
- Xbox services, Event-based vs. title-managed Achievements
author: joannaleecy
ms.topic: article
...
2. **Configuring event-based achievements is too complex.**
    With the event-based Achievements service, an achievement's unlock logic must be configured in Xbox services so that the services know how to interpret the title's stats data and when to unlock the achievement for a user.
...
Yes. The achievements policy is the same for all services. For more information, see [Xbox Live Policies for Achievements and awards](../../../store/policies/pc/live-policies-pc.md#achievements-and-awards).
...
If the title unlocks an achievement while the device is offline, the API automatically queues the offline unlock requests. Unlock requests auto-sync to Xbox services when the device reestablishes network connectivity, similar to the event-based Achievements service's offline experience.
...
The `AchievementUpdate` base event is required by Xbox services for backend purposes.
You can safely ignore this base event.

If your title configures an event by using this base event type, those event writes are ignored by Xbox services.
...
The `XblPlayerDataReset` tool can be used to reset a test user's Xbox player data, including achievements.  Please see the [XblPlayerDataReset documentation](../../../tools/tools-services/live-player-data-reset.md) for details on how to use the tool.
