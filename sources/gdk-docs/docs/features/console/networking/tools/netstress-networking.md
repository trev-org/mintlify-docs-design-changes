...
kindex:
- Xbox services
- Xbox One Network Stress
- Xbox services, Simulate a reduced-capacity network
- Xbox services, Xbox network stress
ms.topic: article
edited: 11/25/2020
ms.date: '08/01/2022'
...
[xbstress (NDA topic)](../../../../tools/tools-console/commandlinetools/xbstress.md) has four preconfigured simulation profiles to easily simulate important network scenarios: minimum, average, excellent, and broken. These profiles are based on figures from various sources that monitor the ongoing quality of internet connections.
...
To use [xbstress (NDA topic)](../../../../tools/tools-console/commandlinetools/xbstress.md) to block all access to the Xbox network, use the IP addresses for the Xbox servers as follows.
...
For more information about the addresses of the Xbox servers, see [Configuring your development network access for developing Microsoft Game Development Kit titles](../../../../gdk-dev/console-dev/dev-kits/settings/configure-dev-network.md).

A host name can also be used instead of an IP address for this command. This host name will be resolved to a single IP address and is therefore only functional for services that are not load-balanced and resolve to a single IP address.
...
To throttle or block communication for a load-balanced HTTP services, Fiddler should be used instead of [xbstress (NDA topic)](../../../../tools/tools-console/commandlinetools/xbstress.md). See [Fiddler on Xbox devkits](fiddler-setup-networking.md) for more information.

## Run a fine-grained simulation of a network

Use `xbstress`to individually control inbound bandwidth, outbound bandwidth, packet loss, and latency. Use these controls when you want more fine-grained control of the simulation parameters. For details, see [xbstress (NDA topic)](../../../../tools/tools-console/commandlinetools/xbstress.md).
...
Using [xbtrace (NDA topic)](../../../../tools/tools-console/commandlinetools/xbtrace.md) to perform on-console network captures while also performing network simulation can result in misleading network captures. Simulation effects (such as packet drops) are implemented after the capture procedure. For example, a capture could indicate that a packet was transmitted, even though it was subsequently dropped by the simulation driver before actually being transmitted over the network.


## See also

 [Stress (xbstress.exe) (NDA topic)](../../../../tools/tools-console/commandlinetools/xbstress.md)
