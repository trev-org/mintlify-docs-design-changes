---
title: "Touch Adaptation Bundle"
description: "Describes the contents of a Touch Adaptation Bundle."
author: M-Stahl
kindex:
- Touch Adaptation Bundle
- Xbox services
edited: 00/00/0000
ms.date: 01/11/2021
ms.topic: article
...
By default, when using [tak.exe](tak-command-line-tool/game-streaming-tak-command-line.md) to `pack` or `serve` a set of assets, they will be optimized to reduce their size and then distributed to the players' device.
...
- `styles`: A [Styles](../../../reference/system/touchadaptationkit/layout/game-streaming-touch-layout-styles.md) object containing a collection of reusable styles which can be referenced in all layouts in the bundle.

A sample `context.json` file that has definitions for defaultDeadzone, named action mappings and state for enabled state for various elements, a set of allowed asset values for the weapons, and a [colors](../../../reference/system/touchadaptationkit/layout/game-streaming-touch-color-palette.md) block containing shared color definitions and system color overrides.
...
There are a set of configuration options for the bundle that can be set using [tak.exe](tak-command-line-tool/game-streaming-tak-command-line.md) when you `pack` or `serve` your bundle:
...
- Version: The content version of the bundle (Example: 1.0.0.0). Each version that is submitted to the Xbox Game Streaming service must be a unique content version.
