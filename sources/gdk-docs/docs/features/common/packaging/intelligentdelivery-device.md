...
At runtime, have your title use the [XSystemGetDeviceType](../../../reference/system/xsystem/functions/xsystemgetdevicetype.md) method to determine which device the title is running on, and it needs to use the appropriate set of assets based on that information.
...
The [XSystemGetDeviceType](../../../reference/system/xsystem/functions/xsystemgetdevicetype.md) values for retail consoles correspond to device specifier tags you'll use in your layout.xml file. Note that `Xbox-Durango` is the device specifier tag for both the Xbox One and Xbox One S.
