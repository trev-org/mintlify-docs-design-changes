...
Use this article to submit a disc request, submit a test disc order, approve test discs, and create a Bill of Materials (BOM). You can only proceed with the instructions outlined in this article if you have the proper permissions. For more information, contact your Microsoft account manager. 
...
![Screenshot of the steps for ordering discs in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/order-process-subway.png)
 ...
 * Reprints
   * For more information about reprints, see [Reorders from the Submit the disc request](#step-sto) section in this article.
...
Smart Delivery is available by using a disc and digital release, or by using full disc support. For more information about Smart Delivery titles for cross-generation gaming, see [Cross-generation gaming overview (NDA topic)](../../console/cross-gen/cross-gen-overview.md).
...
To provide this experience to customers, select an Xbox Series X and an Xbox One package. The option to add both generations becomes available after you've uploaded at least one package of each generation for that product. For more information, see the [Configure disc content for a single product](#step-cdc) section in this article.
 ...
 * A multi-product bundle that's created on a single disc or disc set and includes games and durables.
...
Two additional requirements when creating content for a disc, that aren't requirements for digital release, are manual age ratings and promotional art. For more information about the physical age-rating process, see [XR-017: Title Ratings](../../../store/policies/XR/XR017.md). Promotional art is the image that's used on the disc, and it's required for disc creation. The image on the disc matches the promotional art asset from the listing that's selected during the disc authoring process.
...
![Screenshot of the Disc requests page in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/disc-main.png)
...
![Screenshot of the New disc request dialog in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/new-disc-request-initiate.png)

The following fields are found in the **New disc request** dialog.
 ...
 1. Select one of the available store listing instances from the **Listing instance** dropdown list.  
 ...
 1. Select the checkbox if your product is intended for sale in China.  
...
![Screenshot of the Configure disc content page to edit product information in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/discs-configure-single-product.png)
...
> [!NOTE]
> Disc files that are larger than 50 GB automatically are made into multiple disc binaries. If you're concerned that the size is close to the single disc limit, contact your Microsoft account manager. They will verify the number of disc binaries that are produced before passing the disc to the "test printing" stage. 
>You can also use a `maxDiscs` variable in your disc layout file to guarantee a failure error appears when the size is too large.

 1. Select **Edit** to configure your order name or product listing instance, if necessary.
 1. Select the correct Xbox Series X&#124;S package by using the **Scarlett package** dropdown list. 
    * The **Subset** dropdown list is available when a selected package was uploaded with a disc layout file that contains subsets. Use the **Subset** dropdown list to select any subsets, if available. 
     > [!NOTE] 
 ...
 1. Dual-package options appear when a product is configured to support multiple platforms. Select the correct Xbox One package by using the **Xbox One&nbsp;X package** dropdown list. 
     > [!NOTE] 
     > If packages are only uploaded for Xbox One family of consoles or Xbox Series X&#124;S consoles, a single dropdown list appears on the **Configure disc content** page. 
     >You can select dual packages so that the user receives the native Smart Delivery experience. For more information about Smart Delivery titles for cross-generation gaming, see [Cross-generation gaming overview (NDA topic)](../../console/cross-gen/cross-gen-overview.md).
...
After the disc contents are defined, choose one or more ARs for retail disc manufacturing. This is usually based on the region in which the final product is distributed.
...
Microsoft sends the disc files to the intended AR. However, the partner sends orders to the AR, as well as handles the associated financial arrangements. These processes include the following steps on behalf of the AR and partner.
 ...
 1. The BOM progresses to a release to manufacturing (RTM) state.
 1. Microsoft notifies the AR that the BOM is in an RTM state.
 1. If the AR has an order and a purchase order (PO) that are sent by the partner, the AR can begin final product replication.
...
![Screenshot of the Print discs page in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/print-discs.png)
...
Test discs are sent to the recipients that are listed in the **Ship test discs** page as shown in the following screenshot. This is a preliminary step so that the requester and the Microsoft certification team can evaluate the product prior to submitting the BOM.
...
It's important that you choose the correct location for test discs to be shipped to for completing your internal media verification checks. The partner has the ultimate responsibility of approving and validating that test discs work as expected. Depending on location and test disc production, distribution generally takes three to five business days.

![Screenshot of the Ship test discs page in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/ship-test-discs.png)
...
Use the following steps to complete the **Ship test discs** page in Partner Center.
 ...
 1. If you want to add secondary recipients, select **Add Another Recipient.** Select **Secondary (25 discs),** and then enter the secondary recipient's information in the available fields.
...
The requester can approve or reject discs. Approval is required to proceed with the order. The requester is prompted to approve or reject the order from the **Disc requests** page and **My Disc Request** page as shown in the following screenshot.

![Screenshot of the Disc requests page with the option to approve or reject an order in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/disc-request-row.png)
...
![Screenshot of the Submit disc request page in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/discs-submit-order.png)


### Order summary

After submitting an order, an email is sent to your Microsoft account manager for review.
You can view the order details by selecting the order name on the **Disc requests** page. The **My Disc Request** page includes the **Disc request status** as shown in the following screenshot. You can see which steps are pending or completed, and any errors that are blocking the order process.

![Screenshot of the My Disc Request page in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/discs-order-summary.png)
...
Ordering a bundle product is like ordering a single product. List the subproducts and their associated listing instances when configuring the disc content. After starting a bundle product request, a dialog appears as shown in the following screenshot.

![Screenshot of the New disc request page for bundle product orders in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/bundle-initiate-order.png)

Edit your bundle product order by using the following steps.
 1. Enter a unique name in the **Order Name** field.
 1. Select a listing instance from the **Bundle listing instance** in the dropdown list.
...
![Screenshot of the Configure disc content page for a bundle product in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/bundle-configure-disc-content-1.png)
...
> [!NOTE]
> This policy is subject to change with the new feature of manual age ratings for bundle products. For more information about the physical age rating process, see [XR-017: Title Ratings](../../../store/policies/XR/XR017.md).

![Screenshot of the Configure disc content page in Partner Center.](../../../../media/public-images/packaging/disc-based-games-images/bundle-configure-disc-content-2.png)
