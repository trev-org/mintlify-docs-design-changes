...
Submission Validator isn't a standalone tool that the developer uses. Rather, it's automatically called to check an app whenever the [makepkg pack](../deployment/makepkg.md) or [makepkg2 pack](../deployment/makepkg2.md) command is used. For more information about syntax and command-line use, see the [makepkg](../deployment/makepkg.md) or [makepkg2](../deployment/makepkg2.md) reference pages.  
   
  
Submission Validator runs after the title package is created. Validation failures are recorded in the validation log, which is written to the output location that's specified on the [makepkg](../deployment/makepkg.md) command line. This is the same output location where the finished title package is created. A localized version of the output is also available (in addition to the English output) at the same location if the /validationlanguage parameter is set on [makepkg](../deployment/makepkg.md) or [makepkg2](../deployment/makepkg2.md). The current additional supported validation language is ja-jp.
...
You might also see `<warning>` tags in the log. These generally indicate issues that might require an exception. If you have questions about any warnings that are identified, contact your developer account manager (DAM). Even if you're granted an exception for your title, the `<warning>` tag still appears in the log file. For a full list of log messages, see [Submission Validator Messages](submissionvalidator-messages.md).   
