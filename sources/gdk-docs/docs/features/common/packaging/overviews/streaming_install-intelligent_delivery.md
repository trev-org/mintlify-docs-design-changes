...
- Xbox services, Streaming installation
- Xbox services, Title data
- Xbox services, Intelligent Delivery
- 'Xbox services, Streaming Installation and Intelligent Delivery: an overview'
ms.author: jgup
ms.topic: article
edited: 10/04/2019
ms.date: '08/01/2022'
...
On Xbox One, all titles are run from the hard drive. Content can be delivered from any number of sources, but it's primarily delivered via an optical disc and the cloud&mdash;Blu-ray Disc and Xbox services. 
   
  
To get consumers to start playing games as soon as possible, Xbox One uses a feature called *Streaming Installation*, which allows titles to run before they're completely installed. This feature also allows titles change the order in which content is installed in response to user input or other events.  

> [!NOTE]
> Intelligent Delivery works with both MSIXVC and MSIXVC2 packages. You define chunks the same way in both formats. The key difference with MSIXVC2 is that reordering chunks or moving files between chunks doesn't affect content update sizes. For more information, see [MSIXVC2 packaging overview](packaging-msixvc2-overview.md).
...
After an application package has been created, test Streaming Installation on a console by using `xbapp` `install`. For more information about this command, see [Application Management (xbapp.exe) (NDA topic)](../../../../tools/tools-console/commandlinetools/xbapp.md).  
...
To use Intelligent Delivery, mark chunks in your layout file appropriately. The following code example demonstrates how to mark a chunk so that it's installed only on an Xbox One X console and with the language set to French.
...
[Creating, examining, and testing content updates](../packaging-testing-updates.md)  
Describes how to create, compare, and test content updates in development.

[Content Update best practices (NDA topic)](../packaging-updates.md)  
Describes how to author packages for content update.
