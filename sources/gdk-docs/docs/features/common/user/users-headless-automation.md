...
1.  To add a signed-in user to the console, you can use the [xbuser (NDA topic)](../../../tools/tools-console/commandlinetools/xbuser.md) 
...
2.  To suppress the UI that would usually show when adding a user in the game, you 
    can use the [xbconfig (NDA topic)](../../../tools/tools-console/commandlinetools/xbconfig.md) command-line tool with the `HeadlessXuser` option, shown as follows.
    ```
...
5.  If the game keeps calling `XUserAddAsync`, it eventually exhausts the number 
    ...
    UI).
6.  For testing purposes, games might want to know the number of [test accounts](../../../services/develop/test-accounts/live-setup-testaccounts.md) that are 
