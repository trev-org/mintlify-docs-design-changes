...
kindex:
    - PC DRM license checks
    - Microsoft Store game license validation
    - XStoreQueryGameLicenseResult
    - anti piracy licensing checks
ms.author: cagood
ms.topic: article
edited: '09/08/2021'
ms.date: '08/01/2022'
...
The OS doesn't launch the game unless the user has a valid license for the product including direct ownership, a [shared license](../fundamentals/xstore-product-sharing-model-for-games.md), or a disk license (Xbox console only).
However, on PCs, after a user has a valid license, they can easily decrypt the content, run it outside the context of Microsoft Store, share it with others, and bypass the Microsoft Store licensing system.
...
You can add the code to your startup path, and add specific UI that shows for a failed license check.
...
### Non-Microsoft anti-tamper protection

Although the basic license check example provides some protection, it's common for attackers to bypass this type of protection by modifying the game's executables to bypass this check.
Thus, the next step in securing your game is to use a non-Microsoft anti-tamper product to prevent these lines of attack.
There are options available for GDK PC games.
Work with your anti-tamper vendor to get detailed instructions about how to incorporate this protection into your game.

### Service-side license verification

Anti-tamper protections do a good job at protecting titles at launch, but eventually, many of them are bypassed.
To go a step beyond, online-enabled games can perform a service-side license verification check and block many client-side attacks.
For more information, see [License Tokens Overview](xstore-license-tokens.md).
...
3. The API calls into the licensing service and determines whether the game should return a license to that user.
...
6. The game service validates the signature ensuring that the license came from the Microsoft Store and wasn't modified
