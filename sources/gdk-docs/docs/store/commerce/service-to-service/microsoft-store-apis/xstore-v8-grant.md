...
Only free products can be granted directly to a user's account.
Grant attempts for paid products return an error.
...
| `b2bKey` | `string` | The [User Store ID key](../xstore-requesting-a-userstoreid.md#step-4-create-a-user-store-id-key) that represents the identity of the user for whom you want to grant a product. | Yes |
| `devOfferId` | `string` | A developer-specified offer ID that appears in the Collection item after purchase. | No |
| `language` | `string` | The language of the user. | Yes |
| `market` | `string` | The market of the user. | Yes |
| `orderId` | `GUID` | A GUID generated for the order. This value is unique for the user, but it isn't required to be unique across all orders. | Yes |
...
| Parameter | Type | Description | Required |
| --------- | ---- | ----------- | -------- |
| `clientContext` | `ClientContextV8` | The client contextual information for the order. Assigned to the `clientID` value from the Microsoft Entra ID token. | Yes |
...
| `orderValidityEndTime` | `string` | When the order pricing ends, N/A for free apps. | Yes |
| `orderValidityStartTime` | `string` | When the order pricing starts, N/A for free apps. | Yes |
...
| `billingState` | `string` | The billing state of the order. Set to `Charged` when completed. | No |
...
| `fulfillmentState` | `string` | The state of the item's fulfillment. Set to `Fulfilled` when completed. | No |
...
| ---- | ----- | ---------------- | ----------- |
| 401 | Unauthorized | AuthenticationTokenInvalid | The Microsoft Entra ID access token is invalid. In some cases, the ServiceError details contain more information, such as when the token is expired or the *appid* claim is missing. |
...
| 401 | Unauthorized | InconsistentClientId | The `clientId` claim in the User Store ID key and the `appid` in the Access token don't match. |
