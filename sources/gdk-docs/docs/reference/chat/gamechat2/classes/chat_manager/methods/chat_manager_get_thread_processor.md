...
> [!NOTE] 
> This call will result in a fatal failure if a prior call to [chat_manager::set_thread_affinity_mask](chat_manager_set_thread_affinity_mask.md) has configured the Game Chat internal threads specified by *threadID* to run on more than one processor. In that case, [chat_manager::get_thread_affinity_mask](chat_manager_get_thread_affinity_mask.md) should be used instead.
