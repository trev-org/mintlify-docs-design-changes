...
`XGameStreamingGetLastFrameDisplayed` should be used when trying to determine when an action should have been taken, but wasn't.  In the case where the game needs to determine what was being displayed when an input **was** done by the player, the game should utilize [XGameStreamingGetAssociatedFrame](xgamestreaminggetassociatedframe.md).
