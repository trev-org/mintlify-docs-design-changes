...
> [!IMPORTANT]
> **Deprecated.** This API is deprecated as of the 2604 GDK release and will be removed in a future GDK release. There's no replacement API: titles should use [XGameStreamingGetStreamAddedLatency](xgamestreaminggetstreamaddedlatency.md) for latency-aware gameplay adjustments instead.
...
| --- | --- | --- | 
| E_GAMESTREAMING_NOT_INITIALIZED | 0x89245400 | The XGameStreaming runtime has not been initialized. Call [XGameStreamingInitialize](xgamestreaminginitialize.md) before calling other APIs. |
...
`XGameStreamingGetAssociatedFrame` can only provide information for the game about the frame being displayed when a new input reading is created.  In the case where the game needs to determine what was being displayed when an input may not have been done by the player, the game should utilize [XGameStreamingGetLastFrameDisplayed](xgamestreaminggetlastframedisplayed.md).
...
[Game Streaming Latency Compensation Overview](../../../../features/common/game-streaming/game-streaming-latency-compensation-overview.md)  
