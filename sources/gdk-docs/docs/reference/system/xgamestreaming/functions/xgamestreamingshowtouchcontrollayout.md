...
author: M-Stahl
description: "Request that all connected streaming client devices show a specified touch layout."
ms.date: 04/24/2019
ms.topic: reference
kindex:
...
Request that all connected streaming client devices show a specified touch layout.
...
Name of the touch control layout to display or `nullptr` to display the standard Xbox controller layout.

The id of the touch control layout which should be displayed on the streaming client device. Layout ids are title-specific, and correspond to the ids of the layouts which are provided to the Xbox Game Streaming service as part of the [Touch Adaptation Kit](../../../../features/common/game-streaming/game-streaming-touch-touch-adaptation-kit-overview.md). This string is expected to be UTF-8 encoded.

If `nullptr` is provided, the default set of touch controls will be shown.

Any string which does not correspond to a touch control layout id for the title will not change the state of the touch controls on the client device. The Content Test Application may provide a diagnostic in these cases.
