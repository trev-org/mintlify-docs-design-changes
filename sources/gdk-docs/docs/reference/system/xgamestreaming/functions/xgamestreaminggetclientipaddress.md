...
> [!IMPORTANT]
> **Deprecated.** This API is deprecated as of the 2604 GDK release and will be removed in a future GDK release. On the current Xbox Cloud Gaming streaming stack, this API is non-functional and returns `E_GAMESTREAMING_NO_DATA` on success.
>
> If your title needs to determine whether it's running on an Xbox Game Streaming server, or needs the user's approximate geographic location, use [XSTS token claims](../../../../services/fundamentals/s2s-auth-calls/service-authentication/security-tokens/live-token-claims.md) instead.
...
The buffer that will be populated with the IP address of the specified client.
