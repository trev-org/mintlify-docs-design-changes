...
For an example of an asynchronous provider that implements the `DoWork`, `GetResult`, and `Cleanup` operation codes from `XAsyncOp` and returns a data payload, see the "Async Library" section in [Asynchronous Programming Model](../../../../features/common/async/async-programming-model.md#heading-7).  
