...
[IDStorageQueueX::EnqueueRequest](../interfaces/IDStorageQueueX/methods/idstoragequeuex_enqueuerequest.md) method. The
...
* The combined memory usage of SourceSize (for memory-to-memory requests only) +
  DestinationSize is equal or less than 1GiB (**DSTORAGE_MAX_REQUEST_SIZE**),
  and the memory buffers that are larger than 32MiB must use either 64KiB pages
  or 2MiB pages.

For more information, see the "EnqueueRequest" section of [DirectStorage Overview](../../../../features/console/storage/directstorage/directstorage-overview.md).
