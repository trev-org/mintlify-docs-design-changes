...
Use [XAsyncGetStatus](xasyncgetstatus.md) or the completion callback's `Get\*Result` method to obtain the results of the asynchronous call. If the asynchronous call was canceled these methods return `E_ABORT`. For more information about the implementation pattern of an asynchronous provider, including the `Get\*Result` method, see [Asynchronous Programming Model](../../../../features/common/async/async-programming-model.md).  
