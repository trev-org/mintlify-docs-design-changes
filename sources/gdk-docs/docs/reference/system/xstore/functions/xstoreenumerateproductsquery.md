...
This function is used to enumerate products returned by a function that requests product information. If necessary, once you have called for the enumeration of products use the [XStoreProductsQueryHasMorePages](xstoreproductsqueryhasmorepages.md) function to check for further product information. If there is more product information to be read then use [XStoreProductsQueryNextPageAsync](xstoreproductsquerynextpageasync.md) to read it. See [XStoreQueryAssociatedProductsAsync](xstorequeryassociatedproductsasync.md) for a usage example.  
