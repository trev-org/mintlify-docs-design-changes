...
A specialized version of a Y-axis&ndash;only joystick. Designed for driving games.
...
`visible` - _boolean_, _optional_. Defaults to `true`. Determines if the control is displayed to the player to interact with. To change this during game play, see [Changing touch layouts using game state](../../../../features/common/game-streaming/building-touch-layouts/game-streaming-touch-changing-layouts-game-state.md#change_state).
...
- `activated` - Applied when the player is touching the knob and it's exactly at the 0 value along the axis.
- `activatedDown` - Applied when the player is touching the knob and has moved the knob such that it's in the negative (lower) section of the throttle axis.
- `activatedUp` - Applied when the player is touching the knob and has moved the knob such that it's in the positive (upper) section of the throttle axis.
...
[`knob`](../types/game-streaming-touch-knob.md) - _object_, _optional_. The [`knob`](../types/game-streaming-touch-knob.md) object that specifies the styling of the knob portion of the throttle.

`indicator` - _object_, _optional_. A [stroke](../types/game-streaming-touch-stroke.md) that specifies the styling to apply to the component connecting the knob to the current position on the axis.

**Asset dimensions**

For each of the style objects that accept image assets, a given asset is provided at a base resolution and at 1.5x, 2.0x, 3.0x, and 4.0x scales of that base resolution. The resolution of a given image must be less than or equal to the following maximum resolutions.
...
The `throttle` control allows for the player to provide the variable pressure of a trigger by using a touch control.

The `throttle` control is designed to be placed in the center spot of the wheel&mdash;and to be the only control between that spot and the side of the screen.
...
![User interacting with a throttle](../../../../../media/public-images/game-streaming/touch-control-images/game-streaming-touch-control-images-throttle.gif)
...
#### Example 2: Throttle with styled outline and explicitly defined icons

**Figure 1. Customized throttle control**

![User interacting with a customized throttle](../../../../../media/public-images/game-streaming/touch-control-images/game-streaming-touch-control-images-throttle-customized.gif)
