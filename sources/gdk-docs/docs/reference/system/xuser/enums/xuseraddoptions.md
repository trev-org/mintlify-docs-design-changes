...
- In developer launches, the user that will be the default user is the auto sign in user, which can be specified by setting that user as the auto sign in user in DevHome or by using [xbconfig (NDA topic)](../../../../tools/tools-console/commandlinetools/xbconfig.md) DefaultUser.
