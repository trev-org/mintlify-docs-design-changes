...
| CommunicateUsingText | Check whether or not the user can send a message with text content or an invitation to the target user. This value doesn't change if the player has muted the target user. Use CommunicateUsingVoice instead. This value will be false if you have set your comms to friends only and the target is not a friend. This value will be false if the target user has blocked you. This value will be false if you have set your comms settings to Blocked. |  
