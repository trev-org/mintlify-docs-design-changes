...
This function returns false if a lobby session doesn't exist, or if the Xbox User ID (XUID) specified in `xuid` isn't the host for the lobby session. You can retrieve the host for a lobby session by calling [XblMultiplayerManagerLobbySessionHost](xblmultiplayermanagerlobbysessionhost.md). For more information, see [Multiplayer Manager API overview](../../../../../services/multiplayer/mpm/concepts/live-multiplayer-manager-api-overview.md).
