...
This function retrieves the correlation handle for the game session. The correlation handle serves as an alias for the game session, allowing a game to refer to a game session by using only the correlation handle. The correlation handle can be used to query trace logs for entries that relate to the game session. For more information, see [Multiplayer concepts overview](../../../../../services/multiplayer/concepts/live-multiplayer-concepts.md).
