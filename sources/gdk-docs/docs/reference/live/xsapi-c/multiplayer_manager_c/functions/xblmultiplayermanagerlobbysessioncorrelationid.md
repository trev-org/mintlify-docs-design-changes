...
This function retrieves the correlation handle for the lobby session in Multiplayer Manager (MPM). The correlation handle serves as an alias for the lobby session, allowing a game to refer to a lobby session by using only the correlation handle. The correlation handle can be used to query trace logs for entries that relate to the lobby session. For more information, see [Multiplayer concepts overview](../../../../../services/multiplayer/concepts/live-multiplayer-concepts.md).
