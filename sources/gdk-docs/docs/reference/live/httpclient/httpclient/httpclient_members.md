...
| [HCHttpCallRequestEnableGzipCompression](functions/hchttpcallrequestenablegzipcompression.md) | Enable GZIP compression on the provided body payload. |  
...
| [HCHttpCallResponseSetGzipCompressed](functions/hchttpcallresponsesetgzipcompressed.md) | Enable GZIP compression on the expected response. |  
...
| [HCHttpDisableAssertsForSSLValidationInDevSandboxes](functions/hchttpdisableassertsforsslvalidationindevsandboxes.md) | Disables asserts for SSL validation in console dev sandboxes. |  
| [HCInitialize](functions/hcinitialize.md) | Initializes the library instance. |  
| [HCIsInitialized](functions/hcisinitialized.md) | Returns true if library has been initialized by HCInitialize |  
...
| [HCWebSocketGetBinaryMessageFragmentEventFunction](functions/hcwebsocketgetbinarymessagefragmenteventfunction.md) | Gets the WebSocket binary message fragment handler. |  
...
| [HCWebSocketSetBinaryMessageFragmentEventFunction](functions/hcwebsocketsetbinarymessagefragmenteventfunction.md) | Set the binary message fragment handler. The client functionContext passed to HCWebSocketCreate will also be passed to this handler. |  
| [HCWebSocketSetHeader](functions/hcwebsocketsetheader.md) | Set a header for the WebSocket. |  
| [HCWebSocketSetMaxReceiveBufferSize](functions/hcwebsocketsetmaxreceivebuffersize.md) | Configures how large the WebSocket receive buffer is allowed to grow before passing messages to clients. If a single message exceeds the maximum buffer size, the message will be broken down and passed to clients via multiple calls to the HCWebSocketMessageFunction. The default value is 20kb. |  
...
| --- | --- |  
| [HCCompressionLevel](enums/hccompressionlevel.md) | Defines the compression level that will be used on the compression algorithm. Lower levels perform less compression but have the highest speed in the compression and higher levels perform better compression but have the slowest speed in the compression.|  
| [HCConfigSetting](enums/hcconfigsetting.md) | Defines the config settings value that is passed to the below API's.|  
